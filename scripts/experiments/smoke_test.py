"""Run a small offline smoke test for local experiment configs.

Usage:
    python scripts/experiments/smoke_test.py
    python scripts/experiments/smoke_test.py --gpu
    python scripts/experiments/smoke_test.py --config configs/experiments/local_gpu_smoke.yaml
"""
...
from src.noise_schedule import build_noise_plan
from src.utils import load_config, load_prompts, set_active_config, setup_logging
...
def run_test(config_path: str):
    ...
    noisy_questions = []
    assigned_noise_types = []
    for row, noise_type in build_noise_plan(df, cfg):
        noisy_q = generate_noisy_variant(row["question"], noise_type, prompts, cfg)
        noisy_questions.append(noisy_q)
        assigned_noise_types.append(noise_type)
    ...
    print(f"\n--- Step 7: G-Eval (model: {cfg.get('geval_model', 'gpt-5.4-mini')}) ---\n")
    geval_clean = [geval_answer(r, p, prompts, cfg, question=q)
                   for q, r, p in zip(questions, refs, answers_clean)]
    geval_noisy = [geval_answer(r, p, prompts, cfg, question=q)
                   for q, r, p in zip(questions, refs, answers_noisy)]
    geval_repaired = [geval_answer(r, p, prompts, cfg, question=q)
    ...
    print(f"  {'G-Eval (1-5)':<20} {mean(geval_clean):>10.2f} {mean(geval_noisy):>10.2f} {mean(geval_repaired):>10.2f}")
    print()
    ...
    report = {
        ...
        "geval": {"clean": geval_clean, "noisy": geval_noisy, "repaired": geval_repaired},
    }
    for i, row in df.iterrows():
        report["samples"].append({
            ...
            "noise_type": assigned_noise_types[i],
...
if __name__ == "__main__":
    ...
    if args.config:
        ...
        config_path = str(PROJECT_ROOT / "configs" / "experiments" / "local_gpu_smoke.yaml")
    else:
        config_path = str(PROJECT_ROOT / "configs" / "experiments" / "local_cpu_smoke.yaml")
